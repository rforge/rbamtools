...
// file operations
void BamReader::Close(void) { d->Close(); file_open=false;}
...
bool BamReader::BamReaderPrivate::Jump(int refID, int position) {
    ...
    //  Jump doesn't work with BAM files created by BamWriter because it skips writing RefHasAlignments so they are always FALSE! (WK)
