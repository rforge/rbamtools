...
{
	error("[parse_error] Parse error at line %lld: %s\n", (long long)n_lines, msg);
}
static R_INLINE void append_text(bam_header_t *header, kstring_t *str)
{
	...
	if (x < y) 
    {
        ...
        if ( !header->text ) 
        {
        	error("[append_text] Realloc failed to alloc %ld bytes\n", y);
        }
    }
    // Sanity check
    if ( header->l_text+str->l+1 >= header->n_text )
    {
    	error("[append_text] Error: %ld>=%ld, x=%ld,y=%ld\n",  header->l_text+str->l+1,header->n_text,x,y);
    }
...
int sam_header_parse(bam_header_t *h)
{
	...
	{
		//h->target_name[i] = str_dup(tmp[i]);
		len=strlen(tmp[i]) + 1;
		h->target_name[i] = malloc(len);
		memcpy (h->target_name[i], tmp[i], len);
	}
	free(tmp);
...
int sam_read1(tamFile fp, bam_header_t *header, bam1_t *b)
{
	...
	{ // flag
		long flag;
		char *s;
		ret = ks_getuntil(ks, KS_SEP_TAB, str, &dret); z += (int) str->l + 1;
	...
	{ // tid, pos, qual
		ret = ks_getuntil(ks, KS_SEP_TAB, str, &dret); z += (int) str->l + 1; c->tid = bam_get_tid(header, str->s);
		...
		ret = ks_getuntil(ks, KS_SEP_TAB, str, &dret); z += (int) str->l + 1; c->pos  = isdigit(str->s[0])? (unsigned char) atoi(str->s) - 1 : -1;
		ret = ks_getuntil(ks, KS_SEP_TAB, str, &dret); z += (int) str->l + 1; c->qual = isdigit(str->s[0])? (unsigned char) atoi(str->s) : 0;
		if (ret < 0) return -2;
	}
	{ // cigar
		char *s, *t;
		...
		if (str->s[0] != '*') {
			for (s = str->s; *s; ++s) {
				...
				else if (!isdigit(*s)) parse_error((int64_t) fp->n_lines, "invalid CIGAR character");
			}
			...
			if (*s) parse_error((int64_t) fp->n_lines, "unmatched CIGAR operation");
			c->bin = (short unsigned int) bam_reg2bin( (uint32_t) c->pos, bam_calend(c, bam1_cigar(b)));
			doff += c->n_cigar * 4;
		} else {
			...
			c->bin = (short unsigned int) bam_reg2bin((uint32_t) c->pos, (uint32_t) (c->pos + 1));
		}
	}
	{ // mtid, mpos, isize
		ret = ks_getuntil(ks, KS_SEP_TAB, str, &dret); z += (int) (str->l + 1);
	...
	{ // seq and qual
		...
		if (strcmp(str->s, "*")) {
			...
			if (c->n_cigar && c->l_qseq != (int32_t)bam_cigar2qlen(c, bam1_cigar(b))) {
			  ...
			  parse_error((int64_t) fp->n_lines, "CIGAR and sequence length are inconsistent");
			}
			p = (uint8_t*)alloc_data(b, doff + c->l_qseq + (c->l_qseq+1)/2) + doff;
			memset(p, 0, (size_t) (c->l_qseq+1)/2);
			for (i = 0; i < c->l_qseq; ++i)
				p[i/2] |= (uint8_t) (bam_nt16_table[(int)str->s[i]] << ((uint8_t)4*(1-i%2)) );
		} else c->l_qseq = 0;
		if (ks_getuntil(ks, KS_SEP_TAB, str, &dret) < 0) return -6; // qual
		z += (int) (str->l + 1);
		if (strcmp(str->s, "*") && c->l_qseq != (int64_t) strlen(str->s))
			parse_error((int64_t) fp->n_lines, "sequence and quality are inconsistent");
		p += (c->l_qseq+1)/2;
		if (strcmp(str->s, "*") == 0) for (i = 0; i < c->l_qseq; ++i) p[i] = 0xff;
		else for (i = 0; i < c->l_qseq; ++i) p[i] = (uint8_t) (str->s[i] - 33);
	...
	if (dret != '\n' && dret != '\r') { // aux
		while (ks_getuntil(ks, KS_SEP_TAB, str, &dret) >= 0) {
			...
			if (str->l < 6 || str->s[2] != ':' || str->s[4] != ':')
				parse_error((int64_t) fp->n_lines, "missing colon in auxiliary data");
			key[0] = (uint8_t) str->s[0]; key[1] = (uint8_t) str->s[1];
			type = (uint8_t) str->s[3];
			...
			if (type == 'A' || type == 'a' || type == 'c' || type == 'C') { // c and C for backward compatibility
				...
				*(float*)s = (float)atof(str->s + 9);
				...
				if (type == 'H') { // check whether the hex string is valid
					int i;
					if ((str->l - 5) % 2 == 1) parse_error((int64_t)fp->n_lines, "length of the hex string not even");
					for (i = 0; i < (int) str->l - 5; ++i) {
						int c = toupper(str->s[5 + i]);
						if (!((c >= '0' && c <= '9') || (c >= 'A' && c <= 'F')))
							parse_error((int64_t) fp->n_lines, "invalid hex character");
					}
				}
				...
				*s++ = type;
				...
				char *p;
				if (str->l < 8) parse_error((int64_t) fp->n_lines, "too few values in aux type B");
				...
				*s++ = 'B'; *s++ = (uint8_t) str->s[5];
				...
				else parse_error((int64_t)fp->n_lines, "unrecognized array type");
				s += Bsize * n; doff += size;
			} else parse_error((int64_t)fp->n_lines, "unrecognized type");
			if (dret == '\n' || dret == '\r') break;
		}
	}
	b->l_aux = doff - doff0;
